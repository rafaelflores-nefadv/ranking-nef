...
<div class="min-h-screen bg-[#0a0e1a] p-6">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-white mb-2">Novo Vendedor</h1>
            <p class="text-slate-400">Cadastre um novo Vendedor no sistema</p>
        </div>
        ...
        <div class="bg-slate-900/40 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6">
            <form method="POST" action="{{ route('sellers.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="external_code" name="external_code" value="{{ old('external_code') }}"
                        class="w-full px-4 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('external_code')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                @php
                    ...
                    $initialSectorId = old('sector_id') ?? ($isAdmin ? '' : ($user->sector_id ?? ''));
                            ...
                            @if($teams->count() > 0)
                                @foreach($teams as $team)
                                    <label class="flex items-center space-x-3 p-3 rounded-lg hover:bg-slate-700/50 cursor-pointer transition-colors border border-transparent hover:border-slate-600 team-item"
                                           ...
                                           @if(in_array($team->id, old('teams', []))) style="background-color: rgba(51, 65, 85, 0.3); border-color: rgba(59, 130, 246, 0.3);" @endif>
                                        <input 
                                            type="checkbox" 
                                            name="teams[]" 
                                            value="{{ $team->id }}"
