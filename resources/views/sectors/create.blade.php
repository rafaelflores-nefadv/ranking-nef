...
<div class="min-h-screen bg-[#0a0e1a] p-6">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-white mb-2">Novo Setor</h1>
            <p class="text-slate-400">Crie um novo setor para isolar dados e regras</p>
        </div>
            ...
            <form method="POST" action="{{ route('sectors.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
